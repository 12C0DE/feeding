...
export const ModalData = ({ modData, startDate, uid, timeId, close }) => {
  const [totalLeftSecs, setTotalLeftSecs] = useState(modData.left);
  const [totalRightSecs, setTotalRightSecs] = useState(modData.right);
  const [time, setTime] = useState(modData.timeStmp);
  const timeStr = getTimeLocale(modData.timeStmp);
  const timeSplit = splitTimeArr(timeStr);
  const dateArr = dateSplit(startDate);
  const [isAM, setIsAM] = useState(false);
  const [hours, setHours] = useState(+timeSplit[0]);
  const [mins, setMins] = useState(+timeSplit[1]);

  let leftTime = getTimeFromSecs(modData.left);
  let rightTime = getTimeFromSecs(modData.right);

  const [rightSecs, setRightSecs] = useState(rightTime.secs);
  const [rightMins, setRightMins] = useState(rightTime.mins);

  const [leftSecs, setLeftSecs] = useState(leftTime.secs);
  const [leftMins, setLeftMins] = useState(leftTime.mins);

  useEffect(() => {
    const am = timeSplit[2].indexOf("AM");

    am >= 0 ? setIsAM(true) : setIsAM(false);
  }, []);
